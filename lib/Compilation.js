...
class Compilation extends Tapable {
	...
	constructor(compiler) {
		...
		// debug hook
		hookIntercept("Compilation", this.addEntry);
	}
	...
	addModuleDependencies(
		...
		asyncLib.forEach(
			...
			(item, callback) => {
				...
				const semaphore = this.semaphore; //信号量啊
				semaphore.acquire(() => {
	...
	_addModuleChain(context, dependency, onModule, callback) {
		...
		console.log(
			`read:Compilation _addModuleChain start, constructor = ${
		...
		// this.dependencyFactories = new Map();
		const moduleFactory = this.dependencyFactories.get(Dep); //dependencyFactories在SingleEntryPlugin里面set了moduleFactory
		if (!moduleFactory) {
			throw new Error(
		...
		this.semaphore.acquire(() => {
			console.log(
				`read:Compilation _addModuleChain before moduleFactory.create >>>>>>>>>>>>>>>>>>> context = ${context}`
			);
			moduleFactory.create(
				...
				(err, module) => {
					...
					console.log(
						`read:Compilation _addModuleChain after moduleFactory.create <<<<<<<<<<<<<<<<<<<<`
					);
	...
	addEntry(context, entry, name, callback) {
		//在SingleEntryPlugin调用本方法时：new SingleEntryDependency(entry)，所以entry不再是string了
		//里面代码： const dep = SingleEntryPlugin.createDependency(entry, name);
		// context = /Users/changzhi/zero-plan/webpack-demo, name = app
		console.log(
			`read:Compilation addEntry1 context = ${context}, name = ${name}, entry.request = ${
				entry.request
			}`
		);
		...
		//default true，这里entry是Dep了
		if (entry instanceof ModuleDependency) {
			slot.request = entry.request;
		}

		// TODO webpack 5: merge modules instead when multiple entry modules are supported
		const idx = this._preparedEntrypoints.findIndex(slot => {
		...
		console.log(
			`read:Compilation addEntry2 idx = ${idx}, _preparedEntrypoints.len = ${
				this._preparedEntrypoints.length
			}`
		);
		this._addModuleChain(
			...
			(err, module) => {
				console.log(
					`read:Compilation addEntry3 this._addModuleChain callback err = ${!!err}`
				);
				if (err) {
