...
class WebpackOptionsApply extends OptionsApply {
	...
	process(options, compiler) {
		...
		if (typeof options.target === "string") {
			...
			console.log(
				`debug:WebpackOptionsApply options.target, ${options.target}`
			);
		...
		console.log(
			`debug:WebpackOptionsApply options.externals, ${!!options.externals}` // default, false
		);
		...
		new EntryOptionPlugin().apply(compiler); //2019-05-16 11:33:08，这里面注册了entryOption钩子
		console.log(`debug:WebpackOptionsApply new EntryOptionPlugin()`);
		compiler.hooks.entryOption.call(options.context, options.entry); // 这里添加了入口钩子！！！
		console.log(`debug:WebpackOptionsApply compiler.hooks.entryOption.call`);
		...
		console.log(
			`debug:WebpackOptionsApply options.optimization.moduleIds = ${moduleIds}`
		);
		if (moduleIds) {
		...
		console.log(
			`debug:WebpackOptionsApply options.optimization.chunkIds = ${chunkIds}`
		);
		if (chunkIds) {
		...
		console.log(
			`debug:WebpackOptionsApply options.optimization.minimize = ${
		...
		if (options.optimization.minimize) {
			for (const minimizer of options.optimization.minimizer) {
				console.log(
					`debug:WebpackOptionsApply typeof minimizer = ${typeof minimizer}`
				);
		...
		console.log(
			`debug:WebpackOptionsApply options.performance = ${options.performance}`
		);
