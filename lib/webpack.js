...
const webpack = (options, callback) => {
	...
	console.log(
		`debug:webpack: entry = ${JSON.stringify(
			options.entry
		)}, optimization = ${typeof options.optimization}`
	);
	...
	if (Array.isArray(options)) {
		compiler = new MultiCompiler(options.map(options => webpack(options))); // 递归调用啊
	} else if (typeof options === "object") {
		console.log(
			`debug:webpack new WebpackOptionsDefaulter, now set options.optimization`
		);
		// 里面深拷贝了options，到此options算是处理好了
		options = new WebpackOptionsDefaulter().process(options); // 重点，这里设置了optimization

		compiler = new Compiler(options.context);
		compiler.options = options;
		new NodeEnvironmentPlugin().apply(compiler);

		// 这里所有配置的插件，都依次运行；插件更多可能是注册相应的事件
		if (options.plugins && Array.isArray(options.plugins)) {
			for (const plugin of options.plugins) {
				console.log(`debug:webpack each plugin ${plugin.name}`);
		...
		console.log(
			`debug:webpack process options, typeof optimization = ${typeof options.optimization}`
		);
		compiler.hooks.environment.call();
		compiler.hooks.afterEnvironment.call();
		console.log(`debug:webpack compiler.hooks.afterEnvironment`);
		// 下面这行代码，加了很多很多plugins到compiler上
		compiler.options = new WebpackOptionsApply().process(options, compiler); // 重点，这里初次运行报错的地方
		console.log(`debug:webpack new WebpackOptionsApply().process`);
	} else {
